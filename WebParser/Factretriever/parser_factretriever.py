...
adress = 'C:/Users/arife/Desktop/WhatIsWhat/Information/Factretriever/'

with open(adress + 'factretriever_url.txt') as f:  #url leri txt dosyasında tutuyoruz. 
...
for url in urls :
  ...
  for text_medium in soup.find_all('ul', attrs={'class': 'factsList'}):  #Soruların parse edilidği kısım
      for litag in text_medium.find_all('li'):
       response = litag.get_text()
...
workbook = xlsxwriter.Workbook(adress + 'Factretriever_Information.xlsx')
...
data = pd.read_excel(adress + 'Factretriever_Information.xlsx')
data.sort_values('WHAT/SORU',ascending=True)
data.drop_duplicates(subset ="WHAT/SORU",keep = 'last', inplace = True)
print(data)

ds = pd.DataFrame(data)
ds.to_excel(adress + 'Factretriever_Information.xlsx', sheet_name='Sheet1', index=False)
