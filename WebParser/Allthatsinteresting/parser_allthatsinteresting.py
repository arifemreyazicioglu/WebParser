...
adress = 'C:/Users/arife/Desktop/WhatIsWhat/Information/Allthatsinteresting/'

with open(adress + 'allthatsinteresting_url.txt') as f:  #url leri txt dosyasında tutuyoruz. 
...
for url in urls :
  ...
  for text_medium in soup.find_all('div',attrs = {'class': 'gal-img'}):  #Soruların parse edilidği kısım
      for info in text_medium.find_all('div', attrs = {'class': 'description'}) :
...
workbook = xlsxwriter.Workbook(adress + 'Allthatsinteresting_Information.xlsx')
...
data = pd.read_excel(adress + 'Allthatsinteresting_Information.xlsx')
data.sort_values('WHAT/SORU',ascending=True)
data.drop_duplicates(subset ="WHAT/SORU",keep = 'last', inplace = True)
print(data)

ds = pd.DataFrame(data)
ds.to_excel(adress + 'Allthatsinteresting_Information.xlsx', sheet_name='Sheet1', index=False)
