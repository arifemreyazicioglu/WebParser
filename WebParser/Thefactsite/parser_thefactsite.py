...
adress = 'C:/Users/arife/Desktop/WhatIsWhat/Information/Thefactsite/'

with open(adress + 'thefactsite_url.txt') as f:  #url leri txt dosyasında tutuyoruz. 
...
for url in urls :
  ...
  for text_medium in soup.find_all(attrs = {'class': 'list'}):  #Soruların parse edilidği kısım
      i +=1
...
workbook = xlsxwriter.Workbook(adress + 'Thefactsite_Information.xlsx')
...
data = pd.read_excel(adress + 'Thefactsite_Information.xlsx')
data.sort_values('WHAT/SORU',ascending=True)
data.drop_duplicates(subset ="WHAT/SORU",keep = 'last', inplace = True)
print(data)

ds = pd.DataFrame(data)
ds.to_excel(adress + 'Thefactsite_Information.xlsx', sheet_name='Sheet1', index=False)
