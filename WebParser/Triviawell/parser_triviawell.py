...
adress = 'C:/Users/arife/Desktop/WhatIsWhat/Question/Triviawell/'

with open(adress + 'triviawell_url.txt') as f:  #url leri txt dosyasında tutuyoruz. 
...
for url in urls :
  ...
  for text_medium in soup.find_all('div', attrs={'class': 'text-medium'}):  #Soruların parse edilidği kısım
  ...
  for answer_wrap in soup.find_all('div', attrs={'class': 'answer-wrap'}):  #Cevapların parse edilidği kısım
...
workbook = xlsxwriter.Workbook(adress + 'Triviawell_Question.xlsx')
...
data = pd.read_excel(adress + 'Triviawell_Question.xlsx')
data.sort_values('WHAT/SORU',ascending=True)
data.drop_duplicates(subset ="WHAT/SORU",keep = 'first', inplace = True)
print(data)

ds = pd.DataFrame(data)
ds.to_excel(adress + 'Triviawell_Question.xlsx', sheet_name='Sheet1', index=False)
