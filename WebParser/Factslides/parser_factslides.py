...
adress = 'C:/Users/arife/Desktop/WhatIsWhat/Information/Factslides/'

with open(adress + 'factslides_url.txt') as f:  #url leri txt dosyasında tutuyoruz. 
...
for url in urls :
  ...
  for text_medium in soup.find_all('div', attrs={'class': 'i'}):  #Soruların parse edilidği kısım
...
workbook = xlsxwriter.Workbook(adress + 'Factslides_Information.xlsx')
...
data = pd.read_excel(adress + 'Factslides_Information.xlsx')
data.sort_values('WHAT/SORU',ascending=True)
data.drop_duplicates(subset ="WHAT/SORU",keep = 'last', inplace = True)
print(data)

ds = pd.DataFrame(data)
ds.to_excel(adress + 'Factslides_Information.xlsx', sheet_name='Sheet1', index=False)
