...
adress = 'C:/Users/arife/Desktop/WhatIsWhat/Information/Factscrush/'

with open(adress + 'factscrush_url.txt') as f:  #url leri txt dosyasında tutuyoruz. 
...
for url in urls :
  ...
  for text_medium in soup.find_all('div', attrs={'class': 'post-body post-content'}):  #Soruların parse edilidği kısım
...
workbook = xlsxwriter.Workbook(adress + 'Factscrush_Information.xlsx')
...
data = pd.read_excel(adress + 'Factscrush_Information.xlsx')
data.sort_values('WHAT/SORU',ascending=True)
data.drop_duplicates(subset ="WHAT/SORU",keep = 'last', inplace = True)
print(data)

ds = pd.DataFrame(data)
ds.to_excel(adress + 'Factscrush_Information.xlsx', sheet_name='Sheet1', index=False)
